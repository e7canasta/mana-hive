...
/**
 * Subscribes to scene.fact.v1.> stream and feeds SceneEvents
...
public class SentinelNatsIngest(
    ...
    private fun subscribeToSceneEvents() {
        try {
    ...
    private fun handleSceneEvent(envelope: EventEnvelope) {
        val fact = try {
            SceneEventSerializer.fromJson(envelope.payloadJson)
        } catch (e: Exception) {
            log.warn("Failed to deserialize SceneEvent from envelope {}: {}", envelope.eventId, e.message)
            return
        }
