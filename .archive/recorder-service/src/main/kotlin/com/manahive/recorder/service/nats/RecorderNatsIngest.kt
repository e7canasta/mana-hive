...
public class RecorderNatsIngest(
    ...
    private fun handleEvent(streamName: String, envelope: EventEnvelope) {
        val trigger = when (streamName) {
            "SCENE" -> deserializeSceneEventTrigger(envelope)
    ...
    private fun deserializeSceneEventTrigger(envelope: EventEnvelope): RecordingTrigger? {
        return try {
            val fact = SceneEventSerializer.fromJson(envelope.payloadJson)
            SceneEventTrigger(
                fact = fact,
                bed = fact.bed,
                at = fact.at,
            ...
            log.warn("Failed to deserialize SceneEvent: {}", e.message)
            null
        }
    }
    ...
    private fun deserializeSentinelSignalTrigger(envelope: EventEnvelope): RecordingTrigger? {
        return try {
            val signal = SentinelSignalSerializer.fromJson(envelope.payloadJson)
