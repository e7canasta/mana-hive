 ...
 *
 * ```kotlin
 * val offset = EventOffset.parse("4m30s")  // Duration.ofMinutes(4).plusSeconds(30)
 * val offset = EventOffset.of(Duration.ofSeconds(120))
 * ```
 */
@JvmInline
value class EventOffset(val duration: Duration) : Comparable<EventOffset> {
    ...
    fun plusSeconds(seconds: Long): EventOffset = EventOffset(duration.plusSeconds(seconds))
