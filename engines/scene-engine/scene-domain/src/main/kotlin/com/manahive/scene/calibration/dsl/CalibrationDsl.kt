...
public class CalibrationBuilder {
    ...
    private val confidence = mutableMapOf<StateKind, Confidence>()
    ...
    internal fun build(): SceneCalibration =
        SceneCalibration(table, ConfidenceThresholds(confidence.toMap()), heartbeatTimeout, dwellThresholds.toMap())
}

@SceneDsl
public class ConfidenceBuilder(
    private val kind: StateKind,
    private val confidence: MutableMap<StateKind, Confidence>,
) {
    public infix fun min(value: Double) {
        confidence[kind] = Confidence(value)
    }
}
