...
## 2. The World States — Two Dimensions

### Dimension 1: PersonState (13 states, FSM)
...
### Dimension 2: SceneState (orthogonal flags, compositional)

```
┌─────────────────────────────────────────────────────────────────────┐
│                     SCENE STATE (orthogonal)                         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  PERSONNEL                                                          │
│  ├── staff: PresenceState (NotPresent / Present / InReach)          │
│  └── staffSince: Instant?                                           │
│                                                                      │
│  MOBILITY AIDS                                                       │
│  ├── wheelchair: PresenceState                                       │
│  └── walker: PresenceState                                           │
│                                                                      │
│  BED STATE                                                           │
│  ├── left: RailState (Down / Up / Cover)                            │
│  └── right: RailState (Down / Up / Cover)                           │
│                                                                      │
│  RELATIONSHIP: Cover ⊂ Up (Cover implies Up, but Up ≠ Cover)       │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### State Hierarchy (PresenceState, RailState)

```
PresenceState:  NOT_PRESENT → PRESENT → IN_REACH
                (00)          (01)       (10)

RailState:      DOWN → UP → COVER
                (00)   (01)   (10)

RELATIONSHIP:   IN_REACH ⊂ PRESENT
                COVER ⊂ UP
```
...
### ObservationKind → SceneState (scene events)

```
STAFF_ENTERED      → scene.staff = Present
STAFF_LEFT         → scene.staff = NotPresent
STAFF_IN_REACH     → scene.staff = InReach
WHEELCHAIR_PRESENT → scene.wheelchair = Present
WHEELCHAIR_ABSENT  → scene.wheelchair = NotPresent
WALKER_PRESENT     → scene.walker = Present
WALKER_ABSENT      → scene.walker = NotPresent
BED_RAILS_UP       → scene.bed = scene.bed.withLeft(Up).withRight(Up)
BED_RAILS_DOWN     → scene.bed = scene.bed.withLeft(Down).withRight(Down)
COVER_ON           → scene.bed = scene.bed.withLeft(Cover).withRight(Cover)
COVER_OFF          → scene.bed = scene.bed.withLeft(Up).withRight(Up)
```
...
│  night: NightId                                                     │
│  occupant: ResidentId?                                              │
│  state: PersonState        ← Dimension 1 (FSM)                     │
│  stateSince: Instant                                                 │
│  scene: SceneState          ← Dimension 2 (orthogonal flags)        │
│  sceneSince: Instant                                                 │
│  signal: SignalHealth                                               │
│    ├── monitor: MonitorId                                           │
...
│                                                                      │
│  SCENE STATE FACTS                                                   │
│  SceneStateChanged(bed, night, at, field, from, to)                 │
│  SceneDwellWarning(bed, night, at, field, threshold, since)         │
│  SceneDwellExceeded(bed, night, at, field, threshold, since)        │
│                                                                      │
│  SIGNAL FACTS                                                        │
