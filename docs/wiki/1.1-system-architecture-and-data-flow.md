...
sequenceDiagram
    ...
    participant VIGIA as vigia (Harbor)

    Note over IA,VIGIA: T+0s: Resident leaves bed
    ...
    Note over IA,VIGIA: T+30s: Dwell timeout (on floor)
    ...
    NATS->>VIGIA: Consume Incident
    VIGIA->>VIGIA: RoutingPlanner: Severity -> WARD_BOARD
    VIGIA->>NATS: alarm.event.v1 (AlarmDispatched)
...
- **`AlertLifecycle`**: A `Decider` in Vigia that tracks the state machine of an alert (Created -> Dispatched -> Seen -> Resolved) [README.md64](https://github.com/kerrvisiona-sudo/hive2/blob/f8142c8f/README.md?plain=1#L64-L64)
