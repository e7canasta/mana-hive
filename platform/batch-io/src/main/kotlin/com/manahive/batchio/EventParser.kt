...
object EventParser {
    ...
    fun parseString(content: String): List<Event> {
    ...
    private fun parseLine(line: String, lineNumber: Int): Event? {
        ...
        val offset = parseOffset(trimmed)
            ?: throw IllegalArgumentException("Line $lineNumber: invalid offset format")

        val kindStr = KIND_PATTERN.find(trimmed)?.groupValues?.get(1)
            ?: throw IllegalArgumentException("Line $lineNumber: missing OBS kind")

        val kind = parseKind(kindStr)
            ?: throw IllegalArgumentException("Line $lineNumber: unknown observation kind: $kindStr")

        val confidence = CONFIDENCE_PATTERN.find(trimmed)?.groupValues?.get(1)?.toDoubleOrNull()
            ?: throw IllegalArgumentException("Line $lineNumber: missing or invalid confidence")

        return Event(offset, kind, confidence, lineNumber)
    }

    private fun parseOffset(text: String): Duration? {
